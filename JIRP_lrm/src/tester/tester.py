...
class Tester:
    def __init__(self, learning_params, testing_params, experiment, result_file=None):
        if result_file is None: # in this case, we are running a new experiment
            ...
            for i in range(len(rm_files)):
                ...
                self.reward_machines.append(RewardMachine(self.learning_params.rm_u_max, self.learning_params.rm_preprocess, self.learning_params.rm_tabu_size, self.learning_params.rm_workers, self.learning_params.rm_lr_steps, self.world.get_perfect_rm(), self.learning_params.use_perfect_rm))
    ...
    def run_test(self, step, test_function, rm_learned, rm_true, is_learned, q, *test_args):
        t_init = time.time()
       ...
       # reward_machines = self.get_reward_machines()
        ...
        for task_specification in self.get_task_specifications():
            ...
            reward = test_function(reward_machines, task_params, rm_learned, rm_true, is_learned, q, self.learning_params, self.testing_params, 14, *test_args)
    ...
    def plot_performance(self,steps,p25,p50,p75): #this is the function I need to replicate
        ...
        ax.plot(steps, p50, color='black') #put the 50th percentile in black
