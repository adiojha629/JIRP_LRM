...
def run_aqrm_experiments(alg_name, tester, tester_learned, curriculum, num_times, show_print, show_plots, is_SAT):
    ...
    for t in range(num_times): #goes from 1 to 10;
        ...
        while not curriculum.stop_learning(): #true when current step is more than total learning step (ie when step > 2e6)
            num_episodes += 1
    ...
    title = algorithm_name + " "+task_id+ " "+tester.game_type
    tester.plot_performance(steps_plot,prc_25,prc_50,prc_75,"Reward v Time Step" + tester.game_type,title)
    tester.plot_this(steps_plot,rewards_plot,"Avg Rewards v Time Step"+ tester.game_type,title)
