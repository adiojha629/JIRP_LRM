...
import os.path, time
import numpy as np
from automata_learning.policy_bank import PolicyBank, Policy
from automata_learning.dqn_network import create_net, create_linear_regression, create_target_updates
#import pdb
...
class PolicyBankDQN(PolicyBank):
    def __init__(self, sess, num_actions, num_features, learning_params, hypothesis_reward_machine):
        ...
        super().__init__(sess, num_actions, num_features, learning_params, hypothesis_reward_machine)
    ...
    def learn(self, s1, a, s2, rewards, next_policies, IS_weights, has_been):
        ...
        if self.use_priority:
            # Computing new priorities (sun of the absolute td-errors)
    ...
    def get_best_action(self, rm_id, rm_u, s1, add_noise=False):
        policy = self._get_policy(rm_id, rm_u)
        #print(type(policy))
        #if policy.get_best_action():
        #    print('ok')
        #    print(policy.get_best_action())
        #print()
        #print(type(policy)== <class 'automata_learning.policy_bank.ConstantPolicy'>)
        #pdb.set_trace()
