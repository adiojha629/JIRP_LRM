...
class PolicyBank:
    ...
    """
    def __init__(self, sess, num_actions, num_features, learning_params, hypothesis_reward_machine):
        self.sess = sess
        ...
        t_i = time.time()
        policies_to_add = self._one_policy_per_state(hypothesis_reward_machine)
    ...
    def _one_policy_per_state(self, rm):
        policies_to_add = []
        self.state2policy = {}
        ...
        for ui in range(len(rm.get_states())):
            if rm.is_terminal_state(ui):
                ...
                # IG: later we have to add a check on how policies from previous learnt machines could be reused
                policies_to_add.append("state_"+str(ui))
                self.state2policy[ui] = len(policies_to_add)-1
            ...
            if rm.is_terminal_state(ui):
                ...
                for uj in self.state2policy:
                    ...
                    if rm.is_this_machine_equivalent(ui, reward_machines[0], uj):
                            print("Match: reward machine %d from state %d is equivalent to reward machine %d from state %d"%(i,ui,j,uj))
...
class ConstantPolicy:
    ...
    def _initialize_model(self, value, s2, num_features):
        ...
        #self.best_action = W = tf.constant(random.choice([0,1,2,3]), shape=[num_features, 1], dtype=tf.float64)
    ...
    def get_best_action(self):
        ...
        return self.best_action #see line 172
