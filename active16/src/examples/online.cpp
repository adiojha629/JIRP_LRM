...
list<int> get_CounterExample(int alphabetsize) {
	...
	do {
		ok = true;
                ...
                //cout << "Please enter a counter example: "<<endl;
		cin >> c;
...
bool check_Equivalence(conjecture * cj) {
	...
	if (cj != NULL) {
		...
		//cout << endl << "Conjecture:" << endl << endl;
                std::string s = a->visualize();
                std::ofstream myfile;
                myfile.open ("conjecture.dot");
                myfile << s;
                myfile.close();
                system("dot -Tpng conjecture.dot > outfile.png");
                //system("eog outfile.png");
		                        
                myfile.open ("automaton.txt");
                myfile << cj->write();
                myfile.close();
                //cout << a->visualize();
	}
...
int main(int argc, char**argv) {
	 ...
	 */
        ...
        list<int> empty;
	base.add_knowledge(empty, false);
        for (int i=0;i<alphabet_size;i++) {
		list<int> preli = {i};        
		base.add_knowledge(preli, false);
	}
	do {
	...
	//Display the result on the screen.
        ...
        std::string s = result->visualize();
        std::ofstream myfile;
        myfile.open ("final.dot");
        myfile << s;
        myfile.close();
        system("dot -Tpng conjecture.dot > final.png");
        //system("eog final.png");
        // << result->visualize() << endl;
